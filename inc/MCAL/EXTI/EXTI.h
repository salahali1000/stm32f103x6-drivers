...
typedef struct {
	EXTI_GPIO_Maping_t 		EXTI_Pin ;  //specifies the external interrupt GPIO MAPPING
										//this parameter must be based on @EXTI_define

	uint8_t					Trigger_Case ;  //specifies rising or falling or Both.
											//this parameter must be based on @EXTI_Trigger_define

	uint8_t					IRQ_EN ;  //Enable or disable the EXTI IRQ (this will enable the mask of EXTI and NVIC)
									  //this parameter must be based on @EXTI_IRQ_define

	void(*P_IRQ_CallBck)(void) ;	//Set the function() which will be called when IRQ happen.
